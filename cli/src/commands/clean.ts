...
// a sub-directory of node_modules
const findStdout = shell.exec(`find ${buildpath} -name node_modules -prune -o -name "yarn.lock" -exec dirname {} \\;`, { silent: true }).stdout as string;

// find produces a list of directories delimited by carriage return/newline
const dependencyPaths = findStdout.split(/\r?\n/);

// For each directory, attempt to asynchronously remove dependencies
const dependenciesPromises = dependencyPaths.map(async (dependencyPath: string) => {
  ...
  return new Promise((resolve, reject) => {
    print.info(`Removing ${dependencyPath}...`);
    shell.exec(`rm -rf ${dependencyPath}`, { silent: true, async: true }, (code, stdout, stderr) => {
      if (stderr !== '') {
        print.error(`Failed to remove ${dependencyPath} because \"${stderr}\"`);
        reject();
      } else {
        print.info(`Successfully removed ${dependencyPath}!`);
        resolve();
      }
    });
  });
...
(async () => {
  ...
  await Promise.all(dependenciesPromises)
    ...
    .catch((error) => {
      spinner.stop();
      print.err(error && error.message ? error.message : 'An error occurred while resolving dependencies. [ LunarVerse/cli/src/commands/clean.ts ]');
      process.exit(1);
    });
})();
